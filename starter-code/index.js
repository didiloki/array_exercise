...
exports.uniquifyArray = function () {
    ...
    // Your code here
    result = words.filter(function (item, index) {
...
exports.greatestSumMulti = function () {
    ...
    //code here
    //let sum = 0
    //matrix.forEach.reduce(function(m_ele){
    //  sum += m_ele
    //return sum
    //sum.forEach(function(arrMax){
    //  arrMax = matrix.map(sum)
    //let maxCallback = ( acc, cur ) => Math.max ( acc.arrMax, cur.arrMax );
    // })
    ...
    for(let m = 0; m < matrix.length; m++){
        sum += matrix[m]
        for(let y = 0; y < matrix[m].length; y++){
            sum2 +=arr[x][y]
        }
    }
